...
SEPARATION_FACTOR = 700  # 100, 400, 700, 1000
...
class ConceptConductorPipeline(StableDiffusionPipeline): 
    ...
    def _compute_layout_loss(self, attention_controller, processors_guidance, params_guidance,  custom_attn_guidance_factor=1.0, use_loss_mask=False):
        ...
        separation_factor = SEPARATION_FACTOR #  400, 700, 1000
        base_loss_list = []
        ...
        for processor_name in processors_guidance:
            for param_name in params_guidance:
                ...
                # Compute direct spatial overlap loss
                ...
                if num_models > 1:
                    ...
                    for i in range(num_models):
                        for j in range(i + 1, num_models):
                            overlap = model_attns[i] * model_attns[j]
                        
                            # Sum the overlap across spatial dimensions
                            overlap_loss = torch.sum(overlap, dim=-1).mean()
                            separation_loss = separation_loss + overlap_loss
                            num_pairs += 1
