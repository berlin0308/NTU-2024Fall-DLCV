...
class ConceptConductorPipeline(StableDiffusionPipeline): 
    ...
    def _compute_layout_loss(self, attention_controller, processors_guidance, params_guidance,  custom_attn_guidance_factor=1.0, use_loss_mask=False):
        ...
        print("separation factor :", separation_factor)
        loss_list = []
        base_loss_list = []
        ...
        for processor_name in processors_guidance:
            for param_name in params_guidance:
                ...
                for model_idx in range(len(attention_controller)-1):
                    ...
                    # Clamp values to be positive and add small epsilon
                    attn = torch.clamp(attn, min=0.0) + eps
                    
                    # Softmax normalization instead of direct normalization
                    attn = F.softmax(attn, dim=-1)
                    
                    model_attns.append(attn)
                    
                    # Original layout alignment loss
                    model_loss = (F.mse_loss(attn, ref_attn, reduction='none') * foreground_mask).sum(dim=0).mean()
                    model_losses.append(model_loss)
                
                # Original layout loss term
                ...
                # Add separation loss term
                num_models = len(model_attns)
                if num_models > 1:
                    separation_loss = 0
                    num_pairs = 0
                    ...
                    for i in range(num_models):
                        for j in range(i + 1, num_models):
                            ...
                            attn_i = model_attns[i].view(model_attns[i].size(0), -1)
                            attn_j = model_attns[j].view(model_attns[j].size(0), -1)
                            
                            # Compute overlap using cosine similarity
                            norm_i = torch.norm(attn_i, p=2, dim=1, keepdim=True)
                            norm_j = torch.norm(attn_j, p=2, dim=1, keepdim=True)
                            cos_sim = torch.sum(attn_i * attn_j, dim=1) / (norm_i * norm_j + eps)
                            
                            separation_loss += cos_sim.mean()
                            num_pairs += 1
                    
                    if num_pairs > 0:
                        separation_loss = separation_loss / num_pairs
                        separation_loss_list.append(separation_loss)
        
        # Combine losses
        base_layout_loss = torch.stack(base_loss_list, dim=0).mean(dim=0)
        
        if separation_loss_list:
            separation_loss = torch.stack(separation_loss_list, dim=0).mean(dim=0)
