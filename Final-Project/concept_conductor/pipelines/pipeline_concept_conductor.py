...
class ConceptConductorPipeline(StableDiffusionPipeline): 
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ref_prompt_embeds = ref_prompt_embeds.detach()
        ...
        for rid, custom_prompt in enumerate(custom_prompts):
            if edloras:
                ...
                for idx, (concept_name, concept_embedding) in enumerate(new_concept_embedding.items()):
                    ...
                    self.tokenizer.add_tokens(new_token_names)
                    new_token_ids = [self.tokenizer.convert_tokens_to_ids(token_name) for token_name in new_token_names]
                    self.text_encoder.resize_token_embeddings(len(self.tokenizer))
                    token_embeds = self.text_encoder.get_input_embeddings().weight.data
                    token_embeds[new_token_ids] = concept_embedding.clone().to(device=device, dtype=token_embeds.dtype)
                ...
                custom_prompt_embeds, _ = self.encode_prompt(
                    new_prompts,
                    device,
                    num_images_per_prompt,
                    False
                ...
                custom_negative_prompt_embeds, _ = self.encode_prompt(
                    negative_prompt,
                    device,
                    num_images_per_prompt,
                    False
                )
                custom_prompt_embeds = custom_prompt_embeds.unsqueeze(dim=0).repeat(bs,1,1,1) 
                custom_negative_prompt_embeds = custom_negative_prompt_embeds.unsqueeze(dim=0).repeat(bs,n_cross_attn_layers,1,1) 

                self.tokenizer = copy.deepcopy(original_tokenizer)
                self.text_encoder = copy.deepcopy(original_text_encoder)                                                 

            else:
                custom_prompt_embeds, custom_negative_prompt_embeds = self.encode_prompt(
                    custom_prompt,
                    device,
                    num_images_per_prompt, 
                    self.do_classifier_free_guidance,
                ...
                custom_prompt_embeds = custom_prompt_embeds.repeat(bs,1,1)
                custom_negative_prompt_embeds = custom_negative_prompt_embeds.repeat(bs,1,1)
        ...
        attention_controller = AttentionController(ops_dict=ops_dict, w_min=width//64, h_min=height//64,
        ...
        for  processor_name in self.unet.attn_processors.keys():
            ...
            base_attn_processors[processor_name] = AttnProcessor(attention_controller, processor_name, 0, 
                                                                   cross_attn_idx=cross_attn_idx, use_xformers=True)
                ...
                if requires_attn_guidance:
                    ...
                    for rid in range(len(custom_prompts)):
                        ...
                        custom_noise_pred = custom_unet(
                            ...
                            t,
                            encoder_hidden_states=all_custom_prompt_embeds[rid],
                            timestep_cond=timestep_cond,
                            cross_attention_kwargs=self.cross_attention_kwargs,
                            added_cond_kwargs=added_cond_kwargs,
                            return_dict=False
    ...
    def create_gaussian_kernel(self, kernel_size=3, sigma=1.0, channels=1):
        ...
        # Expand to desired number of channels
    ...
    def _compute_layout_loss(self, attention_controller, processors_guidance, params_guidance,  custom_attn_guidance_factor=1.0, use_loss_mask=False):
        ...
        separation_factor = 0
        print("separation_factor:", separation_factor)
        base_loss_list = []
        separation_loss_list = []
        eps = 1e-8
        gaussian_kernel = self.create_gaussian_kernel(kernel_size=3, sigma=1.0)
        def apply_gaussian_and_self_attention(attn_map, kernel):
            # b, h, hw = attn_map.shape
            # h_dim = int(math.sqrt(hw))
            # Reshape to spatial dimensions
            # ipdb.set_trace()
            # b, h, hw = attn_map.shape  # hw=1280
            # desired_hw = 36 * 36           # 1296
            # if desired_hw > hw:
            #     # pad
            #     pad_len = desired_hw - hw  # 16
            #     # shape: [b, heads, hw+pad_len]
            #     attn_map = F.pad(attn_map, (0, pad_len), value=0.0)
            # elif desired_hw < hw:
            #     # crop
            #     attn_map = attn_map[..., :desired_hw]

            # # Now it is [b, heads, 1296], so you can do:
            # attn_spatial = attn_map.view(b, h, 36, 36)

            # # attn_spatial = attn_map.view(b, h, h_dim, h_dim)
            # kernel = kernel.to(device=attn_spatial.device, dtype=attn_spatial.dtype)
            
            # # Apply Gaussian filter
            # padded = F.pad(attn_spatial, (1, 1, 1, 1), mode='reflect')
            # smoothed = F.conv2d(padded.view(-1, 1, 36+2, 36+2), kernel)
            # smoothed = smoothed.view(b, h, 36, 36)
            
            # # Self dot product
            # smoothed_flat = smoothed.view(b, h, hw)
            self_attention = torch.bmm(attn_map, attn_map.transpose(-2, -1))
        ...
        for processor_name in processors_guidance:
            for param_name in params_guidance:
                ...
                for model_idx in range(len(attention_controller)-1):
                    ...
                    attn = apply_gaussian_and_self_attention(F.softmax(attn + eps, dim=-1), gaussian_kernel)
                    model_attns.append(attn)
                    
                current_loss = model_losses[0]
                if len(model_losses) > 1:
                    current_loss = current_loss + torch.stack(model_losses[1:], dim=0).mean(dim=0) * custom_attn_guidance_factor  
                    base_loss_list.append(current_loss)
                    
                    
                    for i in range(len(model_attns)):
                        for j in range(i + 1, len(model_attns)):
                            intersection = torch.sum(model_attns[i] * model_attns[j], dim=(-2,-1))
                            union = torch.sum(model_attns[i] + model_attns[j] - model_attns[i] * model_attns[j], dim=(-2,-1)) + eps
                            iou = (intersection / union).mean()
                            separation_loss_list.append(iou)

        
        base_layout_loss = torch.stack(base_loss_list, dim=0).mean(dim=0)
        
        
        if separation_loss_list:
            separation_loss = torch.mean(torch.stack(separation_loss_list))
            total_loss = base_layout_loss + separation_factor * separation_loss
            # print("#base layout loss: ", base_layout_loss)
            # print("#separation loss: ", separation_loss)
            # print("#total loss: ", total_loss)
            # print("---------------------------------")
        else:
            total_loss = base_layout_loss
        ...
        #         print("###################model length: ", len(model_losses)) # 4
    ...
    @torch.no_grad()
    def invert(self,
               image: PIL.Image.Image = None, prompt: str = None, prompt_embeds: Optional[torch.Tensor] = None,
        ...
        if image_info is not None:
            ...
            latents_dict = self.ddim_invert(prompt=prompt, image=image, num_inference_steps=num_inference_steps, prompt_embeds=prompt_embeds)
            image_info = {
                'hash_key': hash_key,
                'prompt': prompt,
