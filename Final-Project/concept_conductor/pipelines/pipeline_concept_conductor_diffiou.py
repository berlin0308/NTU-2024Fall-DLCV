        ...
        for processor_name in processors_guidance:
            for param_name in params_guidance:
                ...
                if len(model_losses) > 1:
                    ...
                    separation_loss = torch.tensor(0., device=model_attns[0].device, requires_grad=True)
                    num_pairs = 0
                    
                    for i in range(num_models):
                        for j in range(i + 1, num_models):
                            # Soft intersection: product
                            intersection = model_attns[i] * model_attns[j]
                            
                            # Soft union: sum minus intersection (a + b - a*b)
                            union = model_attns[i] + model_attns[j] - intersection
                            
                            # Soft IoU
                            intersection_sum = torch.sum(intersection, dim=-1)
                            union_sum = torch.sum(union, dim=-1) + eps
                            
                            soft_iou = intersection_sum / union_sum
                            
                            # Average across heads/tokens
                            separation_loss = separation_loss + soft_iou.mean()
                            num_pairs += 1
