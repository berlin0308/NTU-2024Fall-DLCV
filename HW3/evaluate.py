...
def main(args):
    ...
    unmatched_preds = set(predictions.keys()) - set(gts.keys())
    unmatched_gts = set(gts.keys()) - set(predictions.keys())

    if unmatched_preds:
        print(f"Extra IDs in predictions not in ground truth: {unmatched_preds}")
    if unmatched_gts:
        print(f"Missing IDs in predictions that are in ground truth: {unmatched_gts}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images_root", default="hw3_data/p1_data/images/val/", help="Image root")
    parser.add_argument("--annotation_file", default="hw3_data/p1_data/val.json", help="Annotation json file")
