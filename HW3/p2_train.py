...
from p2_legacy.p2_dataset import ICDataset, Image_dataset
from p2_legacy.p2_model import ImageCaptioningTransformer, GradualWarmupScheduler, ImageCaptioningTransformerLoRA
...
class ScheduledSampling:
    def __init__(self, total_steps, enabled=True) -> None:
        ...
        self.enabled = enabled  # Fix: add this line to initialize 'enabled' attribute
...
def main(args):
    ...
    if not args.lora:
        print("Build model without LoRA... ")
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model.state_dict(), args.output_dir / f"model_ep{epoch}.pth")
        ...
        for cnt, (img, name) in tqdm(enumerate(valid_set), total=len(valid_set)):
            ...
            with torch.no_grad():
                with torch.autocast(device_type=amp_device_type, dtype=amp_dtype, enabled=amp_enable):
                    output_ids = model.beam_search(img.to(args.device), beams=3, max_length=30, device=args.device)
        ...
        if clip_score > history_best_CLIPscore:
            ...
            torch.save(model.state_dict(),
                       args.output_dir / "CLIPscore" / "Best_model.pth")
            json.dump(model.config, (args.output_dir / "CLIPscore" /
        ...
        for image_name, text in preds.items():
            print(f"Ans: {annotations[img2id[image_name]]}")
            print(f"Pred: {text}")
        ...
        if CIDEr_score > history_best_CIDEr:
            ...
            torch.save(model.state_dict(),
                       args.output_dir / "CIDEr" / "Best_model.pth")
            json.dump(model.config, (args.output_dir / "CIDEr" /
                                     f"model_config.json").open(mode='w'), indent=4)
...
def parse():
    ...
    parser.add_argument("--output_dir",
                        type=pathlib.Path, default="p2_outputs")
    ...
    parser.add_argument('--model', type=str,
                        default='vit_large_patch14_clip_224.laion2b')
...
if __name__ == '__main__':
    ...
    if args.lora:
        args.output_dir = args.output_dir / f"lora_b{args.batch_size}_lr{args.lr}_{args.tag}"
    else:
        args.output_dir = args.output_dir / f"b{args.batch_size}_lr{args.lr}_{args.tag}"

    args.output_dir.mkdir(exist_ok=True, parents=True)

    (args.output_dir / "CLIPscore").mkdir(exist_ok=True, parents=True)
    (args.output_dir / "CIDEr").mkdir(exist_ok=True, parents=True)
    main(args)